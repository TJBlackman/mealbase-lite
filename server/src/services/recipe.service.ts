import {
  ...
  saveRecipeDAL,
  editRecipeDAL,
  getLikeRecordsByUserIdAndRecipeIds,
  likeRecipeDAL,
  unLikedRecipeDAL,
  getLikeRecordsByRecipeId,
  countRecipesDAL,
  getUserLikedRecipeRecords,
  getRandomRecipes,
} from "../DAL/recipe.dal";
...
export const getRecipesService = async (query: RecipeQuery, user: JWTUser) => {
  ...
  if (query.randomize === "1") {
    ...
    const _recipes = await getRandomRecipes(num);
    return {
      totalCount: _recipes.length,
      recipes: _recipes,
    };
  }
  ...
  if (user) {
    ...
    const likeRecords = await getLikeRecordsByUserIdAndRecipeIds({
      userId: user._id,
      recipeIds,
    });
...
export const likeRecipeService = async (
  ...
  const alreadyLiked = await getLikeRecordsByUserIdAndRecipeIds({
...
export const unLikedRecipeService = async (
  data: { recipeId: string },
  user: JWTUser
) => {
  const existingRecipe = await queryRecipeDAL({ _id: data.recipeId });
  ...
  const recipeIsLiked = await getLikeRecordsByUserIdAndRecipeIds({
    recipeIds: [data.recipeId],
    userId: user._id,
  });
